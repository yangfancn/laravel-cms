<template>
  <div :class="{ comment: true, hasChildren: children_count, active: treeLineActive }" class="mb-5">
    <div class="info">
      ...
      <div class="show-rating" v-if="data.rating">
        <!-- <rating v-model="data.rating" :disable="true" :size="14" /> -->
        <strong>{{ data.rating }}</strong>
      </div>
    </div>
    <div class="content">
      <div class="text prose-md prose max-w-none" v-html="data.content"></div>
      <div class="mt-4 flex items-center">
        ...
        />
        <button class="btn ml-4 rounded-full btn-sm btn-neutral" @click="inputting = !inputting">
          <svg
    ...
    <div class="children" v-if="children_count">
      ...
      <div class="show-more" v-if="children.length < children_count" @click="loadMore">
        ...
        <div class="text">
          ...
          <span>{{ trans("layout.show-more") }}</span>
        </div>
      </div>
    </div>
  </div>
