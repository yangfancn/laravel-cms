<template>
  ...
  >
    ...
    <div class="toolbar hidden flex-wrap items-center justify-between" @mousedown.prevent>
      ...
      <div class="actions flex items-center justify-end">
        <button class="cancel btn btn-accent mr-2 cursor-pointer" @mousedown.prevent @click="cancel">Cancel</button>
        <button
  ...
  <p class="error-message text-error mt-2 mb-4 text-sm" v-if="errorMessage">{{ errorMessage }}</p>
</template>
...
const submitComment = inject<(content: string, id: number | null) => Promise<CommentItem>>("submitComment")!
