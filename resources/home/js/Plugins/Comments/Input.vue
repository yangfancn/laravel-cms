<template>
  <div
    ...
    class="mt-10 rounded-lg border p-3"
  >
    <div class="input-rating" v-if="useRating">
      <Rating v-model="rating" :size="16" />
      {{ rating }}
    </div>
    <div class="editor">
      <editor-content :editor="editor" />
    </div>
    <div class="toolbar hidden flex-wrap items-center justify-between" @mousedown.prevent>
      <div class="format flex flex-nowrap items-center justify-start">
      ...
      <div class="actions flex items-center justify-end">
        <button class="cancel btn mr-2 cursor-pointer btn-accent" @mousedown.prevent @click="cancel">Cancel</button>
        <button
          class="submit btn cursor-pointer bg-secondary text-secondary-content"
          @mousedown.prevent
          @click="submit"
        >
          Comment
  ...
  <p class="error-message mt-2 mb-4 text-sm text-error" v-if="errorMessage">{{ errorMessage }}</p>
</template>
...
const submitComment =
  inject<(content: string, id: number | null, rating: number | null) => Promise<any>>("submitComment")!

const submit = () => {
  submitComment(editor?.value?.getHTML() ?? "", props.commentId ?? null, rating.value)
